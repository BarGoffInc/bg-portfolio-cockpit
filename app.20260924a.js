...
(function () {
  ...
  /* ---- NAV series chart (Overview + Daily) ---- */
  let __NAV_SERIES__ = null;
  let __NAV_SHOW_FLOW__ = false;

  async function loadNavSeries() {
    const res = await fetch(dataUrl("daily/nav-series.json"), {
      cache: FORCE_BUST ? "no-store" : "default",
    });
    if (!res.ok) throw new Error("Could not load daily/nav-series.json (" + res.status + ")");
    return res.json();
  }
  ...
  function renderNavSeriesChart(series) {
    __NAV_SERIES__ = series;
    const svg = $("#nav-series-svg");
    const tip = $("#nav-series-tip");
    const sub = $("#nav-series-sub");
    const flowToggle = $("#nav-show-flow");
    if (!svg) return;

    if (flowToggle && !flowToggle.dataset.ready) {
      flowToggle.dataset.ready = "1";
      flowToggle.checked = !!__NAV_SHOW_FLOW__;
      flowToggle.addEventListener("change", () => {
        __NAV_SHOW_FLOW__ = !!flowToggle.checked;
        if (__NAV_SERIES__) renderNavSeriesChart(__NAV_SERIES__);
      });
    }

    const points = (series && series.points) || [];
    const flowDays = (series && series.flow_days) || [];
    const trustedN = points.filter((p) => p.method === "trusted" || p.provisional === false).length;
    const reconN = points.filter((p) => p.method === "reconstructed" || p.provisional === true).length;
    if (sub) {
      const last = points.length ? points[points.length - 1] : null;
      sub.textContent = points.length
        ? points.length + " days · " + trustedN + " trusted · " + reconN + " reconstructed" +
          (last ? " · latest " + last.day + " " + fmtCompactUSD(last.nav_usd) : "")
        : "No NAV marks yet";
    }

    const W = 720, H = 280;
    const pad = { l: 58, r: 16, t: 18, b: __NAV_SHOW_FLOW__ ? 118 : 78 };
    const plotW = W - pad.l - pad.r;
    const navH = __NAV_SHOW_FLOW__ ? 110 : 140;
    const pnlH = 48;
    const flowH = 40;
    const navTop = pad.t;
    const pnlTop = navTop + navH + 10;
    const flowTop = pnlTop + pnlH + 10;

    if (!points.length) {
      svg.setAttribute("viewBox", "0 0 " + W + " " + H);
      svg.innerHTML = `<text x="${W/2}" y="${H/2}" text-anchor="middle" fill="#7a8a92" font-size="13">No NAV points yet</text>`;
      return;
    }

    const navs = points.map((p) => Number(p.nav_usd));
    const minNav = Math.min(...navs);
    const maxNav = Math.max(...navs);
    const navPad = Math.max(500, (maxNav - minNav) * 0.08 || 1000);
    const y0 = minNav - navPad;
    const y1 = maxNav + navPad;

    let xDays = points.map((p) => p.day);
    if (__NAV_SHOW_FLOW__ && flowDays.length) {
      const set = new Set(xDays);
      flowDays.forEach((d) => { if (d.day) set.add(d.day); });
      xDays = Array.from(set).sort();
    }
    const dayIndex = {};
    xDays.forEach((d, i) => { dayIndex[d] = i; });
    const nX = Math.max(1, xDays.length - 1);
    const xAt = (day) => pad.l + (dayIndex[day] / nX) * plotW;
    const yNav = (v) => navTop + navH - ((v - y0) / (y1 - y0 || 1)) * navH;

    const pnls = points.map((p) => (p.day_pnl_usd != null ? Number(p.day_pnl_usd) : 0));
    const maxPnlAbs = Math.max(1, ...pnls.map((v) => Math.abs(v)));
    const yPnl = (v) => {
      const mid = pnlTop + pnlH / 2;
      return mid - (v / maxPnlAbs) * (pnlH / 2 - 2);
    };

    let maxFlowAbs = 1;
    if (__NAV_SHOW_FLOW__) {
      maxFlowAbs = Math.max(1, ...flowDays.map((d) => Math.abs(Number(d.net_flow_usd) || 0)));
    }
    const yFlow = (v) => {
      const mid = flowTop + flowH / 2;
      return mid - (v / maxFlowAbs) * (flowH / 2 - 2);
    };

    function isTrusted(p) {
      return p.method === "trusted" || p.provisional === false;
    }

    // polyline for all points (continuous series)
    const poly = points
      .map((p, i) => (i === 0 ? "M" : "L") + xAt(p.day).toFixed(1) + "," + yNav(Number(p.nav_usd)).toFixed(1))
      .join(" ");

    // dashed overlay segments between non-trusted runs (visual cue): draw full line solid-muted + trusted connectors solid
    const ticks = 4;
    let grid = "";
    for (let i = 0; i <= ticks; i++) {
      const v = y0 + ((y1 - y0) * i) / ticks;
      const y = yNav(v);
      grid += `<line x1="${pad.l}" y1="${y.toFixed(1)}" x2="${W - pad.r}" y2="${y.toFixed(1)}" stroke="rgba(16,40,50,0.08)" stroke-width="1"/>`;
      grid += `<text x="${pad.l - 6}" y="${(y + 3).toFixed(1)}" text-anchor="end" fill="#7a8a92" font-size="10" font-family="ui-sans-serif,system-ui">${fmtCompactUSD(v)}</text>`;
    }

    let pnlBars = `<line x1="${pad.l}" y1="${(pnlTop + pnlH / 2).toFixed(1)}" x2="${W - pad.r}" y2="${(pnlTop + pnlH / 2).toFixed(1)}" stroke="rgba(16,40,50,0.12)" stroke-width="1"/>`;
    const barW = Math.max(3, Math.min(14, plotW / Math.max(points.length * 1.2, 8)));
    points.forEach((p) => {
      if (p.day_pnl_usd == null) return;
      const v = Number(p.day_pnl_usd);
      const x = xAt(p.day);
      const y = yPnl(v);
      const mid = pnlTop + pnlH / 2;
      const top = Math.min(y, mid);
      const h = Math.max(2, Math.abs(y - mid));
      const fill = v >= 0 ? "#2a9d8f" : "#c85e52";
      const op = isTrusted(p) ? "0.9" : "0.45";
      pnlBars += `<rect class="nav-pnl-bar" data-day="${p.day}" x="${(x - barW / 2).toFixed(1)}" y="${top.toFixed(1)}" width="${barW.toFixed(1)}" height="${h.toFixed(1)}" rx="1.5" fill="${fill}" opacity="${op}"/>`;
    });

    let flowBars = "";
    if (__NAV_SHOW_FLOW__) {
      flowBars += `<line x1="${pad.l}" y1="${(flowTop + flowH / 2).toFixed(1)}" x2="${W - pad.r}" y2="${(flowTop + flowH / 2).toFixed(1)}" stroke="rgba(16,40,50,0.12)" stroke-width="1"/>`;
      const fBarW = Math.max(2, Math.min(8, plotW / Math.max(flowDays.length, 8)));
      flowDays.forEach((d) => {
        const v = Number(d.net_flow_usd) || 0;
        if (!v) return;
        if (dayIndex[d.day] == null) return;
        const x = xAt(d.day);
        const y = yFlow(v);
        const mid = flowTop + flowH / 2;
        const top = Math.min(y, mid);
        const h = Math.max(2, Math.abs(y - mid));
        flowBars += `<rect class="nav-flow-bar" data-day="${d.day}" x="${(x - fBarW / 2).toFixed(1)}" y="${top.toFixed(1)}" width="${fBarW.toFixed(1)}" height="${h.toFixed(1)}" rx="1" fill="#8d9aa3" opacity="0.55"/>`;
      });
    }
    ...
    points.forEach((p) => {
      const x = xAt(p.day);
      const y = yNav(Number(p.nav_usd));
      const trusted = isTrusted(p);
      if (trusted) {
        hits += `<circle class="nav-dot trusted" data-day="${p.day}" cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="5" fill="#147b83" stroke="#fff" stroke-width="1.75"/>`;
      } else {
        hits += `<circle class="nav-dot recon" data-day="${p.day}" cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="3.25" fill="none" stroke="#147b83" stroke-width="1.5" opacity="0.75"/>`;
      }
      hits += `<rect class="nav-hit" data-day="${p.day}" x="${(x - 8).toFixed(1)}" y="${navTop}" width="16" height="${navH + pnlH + 20}" fill="transparent"/>`;
    });

    // sparse x labels: first, last, and trusted days
    let xLabels = "";
    const labelDays = new Set([points[0].day, points[points.length - 1].day]);
    points.forEach((p) => { if (isTrusted(p)) labelDays.add(p.day); });
    // also ~monthly mid if long
    if (points.length > 20) {
      labelDays.add(points[Math.floor(points.length / 2)].day);
    }
    Array.from(labelDays).forEach((day) => {
      const x = xAt(day);
      const label = String(day).slice(5);
      xLabels += `<text x="${x.toFixed(1)}" y="${(pnlTop + pnlH + (__NAV_SHOW_FLOW__ ? flowH + 22 : 16)).toFixed(1)}" text-anchor="middle" fill="#7a8a92" font-size="10" font-family="ui-sans-serif,system-ui">${label}</text>`;
    });

    const labels = `
      <text x="${pad.l}" y="${(navTop - 4).toFixed(1)}" fill="#5b6b73" font-size="10" font-weight="600" font-family="ui-sans-serif,system-ui">Book NAV</text>
      <text x="${W - pad.r}" y="${(navTop - 4).toFixed(1)}" text-anchor="end" fill="#8d9aa3" font-size="9" font-family="ui-sans-serif,system-ui">● trusted  ○ reconstructed</text>
      <text x="${pad.l}" y="${(pnlTop - 2).toFixed(1)}" fill="#5b6b73" font-size="10" font-weight="600" font-family="ui-sans-serif,system-ui">Day P/L (vs prior series point)</text>
      ${__NAV_SHOW_FLOW__ ? `<text x="${pad.l}" y="${(flowTop - 2).toFixed(1)}" fill="#8d9aa3" font-size="10" font-weight="600" font-family="ui-sans-serif,system-ui">Net buys − sells (flows)</text>` : ""}
    `;

    const vbH = __NAV_SHOW_FLOW__ ? 300 : 260;
    svg.setAttribute("viewBox", "0 0 " + W + " " + vbH);
    svg.innerHTML =
      labels +
      grid +
      `<path d="${poly}" fill="none" stroke="#147b83" stroke-width="2" stroke-linejoin="round" stroke-linecap="round" opacity="0.55" stroke-dasharray="5 4"/>` +
      `<path d="${poly}" fill="none" stroke="#147b83" stroke-width="2.25" stroke-linejoin="round" stroke-linecap="round" opacity="0.35"/>` +
      pnlBars +
      flowBars +
      hits +
      xLabels;
    ...
    const byDay = {};
    points.forEach((p) => { byDay[p.day] = p; });
    const flowByDay = {};
    flowDays.forEach((d) => { flowByDay[d.day] = d; });

    function showTip(day, clientX, clientY) {
      if (!tip) return;
      const p = byDay[day];
      if (!p) return;
      const f = flowByDay[day];
      const pnl = p.day_pnl_usd;
      const trusted = isTrusted(p);
      tip.hidden = false;
      tip.innerHTML =
        `<strong>${escapeHtml(day)}</strong>` +
        `<span>NAV ${fmtUSD(p.nav_usd)}</span>` +
        `<span class="${pnlClass(pnl)}">Day Δ ${pnl != null ? fmtUSD(pnl, { cents: false }) : "—"}</span>` +
        `<span class="text-muted">${trusted ? "trusted print" : "reconstructed estimate"}</span>` +
        (f && __NAV_SHOW_FLOW__
          ? `<span class="text-muted">Net flow ${fmtUSD(f.net_flow_usd, { cents: false })}</span>`
          : "");
      ...
      tip.style.top = Math.min(top, rect.height - 80) + "px";
    }
    function hideTip() {
      if (tip) tip.hidden = true;
    }

    svg.querySelectorAll(".nav-hit, .nav-dot, .nav-pnl-bar").forEach((el) => {
      el.addEventListener("mousemove", (ev) => showTip(el.getAttribute("data-day"), ev.clientX, ev.clientY));
      el.addEventListener("mouseleave", hideTip);
      el.addEventListener("touchstart", (ev) => {
        const t = ev.touches && ev.touches[0];
        if (t) showTip(el.getAttribute("data-day"), t.clientX, t.clientY);
      }, { passive: true });
    });
  }

  async function bootNavSeries() {
    try {
      const series = await loadNavSeries();
      renderNavSeriesChart(series);
    } catch (err) {
      console.warn(err);
      const sub = $("#nav-series-sub");
      if (sub) sub.textContent = "nav-series unavailable";
      const svg = $("#nav-series-svg");
      if (svg) {
        svg.innerHTML = `<text x="360" y="140" text-anchor="middle" fill="#7a8a92" font-size="13">Could not load nav-series.json</text>`;
      }
    }
  }
  ...
  /* ---- Daily EOD archive ---- */
  let __DAILY_INDEX__ = null;
  let __DAILY_DAY__ = null;
  const DAILY_LINE_CAP = 200;

  async function loadDailyIndex() {
    const res = await fetch(dataUrl("daily/index.json"), {
      cache: FORCE_BUST ? "no-store" : "default",
    });
    if (!res.ok) throw new Error("Could not load daily/index.json (" + res.status + ")");
    return res.json();
  }

  async function loadDailyActivity(day) {
    const res = await fetch(dataUrl("daily/" + day + "/activity.json"), {
      cache: FORCE_BUST ? "no-store" : "default",
    });
    if (!res.ok) throw new Error("Could not load activity for " + day + " (" + res.status + ")");
    return res.json();
  }

  function dailyQueryDay() {
    try {
  ...
  function defaultDailyDay(index) {
    const q = dailyQueryDay();
    if (q && (index.days || []).some((d) => d.day === q)) return q;
    const withAct = (index.days || []).find((d) => d.has_activity);
    if (withAct) return withAct.day;
    return index.latest_day || ((index.days || [])[0] && index.days[0].day) || null;
  }

  function populateDailySelect(index) {
    const sel = $("#daily-day");
    if (!sel) return;
    const tradeOnly = $("#daily-trade-only");
    const onlyTrade = !!(tradeOnly && tradeOnly.checked);
    let days = (index.days || []).slice();
    if (onlyTrade) days = days.filter((d) => d.has_activity);
    const prefer = sel.value || __DAILY_DAY__ || dailyQueryDay() || "";
    sel.innerHTML =
      days
        .map((d) => {
          const label =
            d.day + (d.has_activity ? " · " + (d.event_count || 0) + " ev" : " · quiet");
          return `<option value="${escapeHtml(d.day)}">${escapeHtml(label)}</option>`;
        })
        .join("") || `<option value="">No days</option>`;
    if (prefer && [...sel.options].some((o) => o.value === prefer)) {
      sel.value = prefer;
    } else {
      const def = defaultDailyDay({
        days: days.length ? days : index.days || [],
        latest_day: index.latest_day,
      });
    ...
    if (!sel.dataset.ready) {
      ...
      sel.addEventListener("change", () => {
        ...
        loadAndRenderDailyDay(day);
      });
    }
    if (tradeOnly && !tradeOnly.dataset.ready) {
      tradeOnly.dataset.ready = "1";
      tradeOnly.addEventListener("change", () => {
        populateDailySelect(__DAILY_INDEX__ || index);
        const day = sel.value;
        if (day) {
          try {
            const url = new URL(location.href);
  ...
  async function loadAndRenderDailyDay(day) {
    __DAILY_DAY__ = day;
    try {
      const act = await loadDailyActivity(day);
      renderDaily(act);
    } catch (err) {
      console.error(err);
      toast("Failed to load " + day + ": " + (err && err.message ? err.message : err));
      const body = $("#daily-body");
      if (body) {
        body.innerHTML =
          `<tr><td colspan="8" class="empty">Failed to load activity.json for ${escapeHtml(
            day
          )}</td></tr>`;
      }
    }
  ...
  function renderDaily(act) {
    const day = act.day || __DAILY_DAY__ || "—";
    const hero = $("#daily-hero");
    if (hero) {
      const platforms = (act.platforms || []).join(", ") || "—";
      const pnl = act.known_realized_pnl_usd;
      hero.innerHTML = `
        <div class="hero-top">
          <div>
            <p class="label">Selected day (ET)</p>
            <h2 class="total tabular" style="font-size:clamp(28px,5vw,42px)">${escapeHtml(
              day
            )}</h2>
            <p class="meta">${escapeHtml(platforms)}</p>
          </div>
        </div>
        <div class="chips">
          <div class="chip" role="group">
            <span class="name">Buy USD</span>
            <strong class="val tabular">${fmtUSD(act.buy_usd, { cents: false })}</strong>
          </div>
          <div class="chip" role="group">
            <span class="name">Sell USD</span>
            <strong class="val tabular">${fmtUSD(act.sell_usd, { cents: false })}</strong>
          </div>
          <div class="chip" role="group">
            <span class="name">Known realized P/L</span>
            <strong class="val tabular ${pnlClass(pnl)}">${
              pnl != null ? fmtUSD(pnl, { cents: false }) : "—"
            }</strong>
          </div>
          <div class="chip" role="group">
            <span class="name">Events</span>
            <strong class="val tabular">${Number(
              act.event_count != null ? act.event_count : (act.lines || []).length
            ).toLocaleString()}</strong>
          </div>
          <div class="chip" role="group">
            <span class="name">Platforms</span>
            <strong class="val tabular" style="font-size:13px">${escapeHtml(platforms)}</strong>
          </div>
        </div>`;
    }
    ...
    if (note) {
      const parts = [
        "Known realized P/L only when the source provides it — never invented.",
        "Transfers and other non-trade sides are listed but excluded from buy/sell totals.",
      ];
      if (act.coverage_notes && act.coverage_notes.length) {
        parts.push(act.coverage_notes.join(" "));
      }
      if (act.gaps && act.gaps.length) {
        parts.push("Gaps: " + act.gaps.join("; "));
      }
      note.textContent = parts.join(" ");
    }
    ...
    const trunc = $("#daily-trunc");
    if (!lines.length) {
      body.innerHTML = `<tr><td colspan="8" class="empty">No activity lines for ${escapeHtml(
        day
      )}</td></tr>`;
      if (trunc) trunc.textContent = "";
      return;
    }
    const truncated = lines.length > DAILY_LINE_CAP;
    const shown = lines.slice(0, DAILY_LINE_CAP);
    if (trunc) {
      trunc.textContent = truncated
        ? "Showing first " + DAILY_LINE_CAP + " of " + lines.length + " lines."
        : lines.length + " lines.";
    }
    body.innerHTML = shown
      .map((ln) => {
        const pnl = ln.realized_pnl_usd;
        return `<tr>
          <td class="mono text-muted">${escapeHtml(formatTs(ln.ts))}</td>
          <td>${escapeHtml(ln.platform || "")}</td>
          <td><strong>${escapeHtml(String(ln.symbol || "—"))}</strong></td>
          <td>${escapeHtml(ln.side || "")}</td>
          <td class="right mono">${ln.qty != null ? fmtNum(ln.qty, 6) : "—"}</td>
          <td class="right mono">${ln.price != null ? fmtUSD(ln.price) : "—"}</td>
          <td class="right mono">${
            ln.notional_usd != null ? fmtUSD(ln.notional_usd, { cents: false }) : "—"
          }</td>
          <td class="right mono ${pnlClass(pnl)}">${
            pnl != null ? fmtUSD(pnl, { cents: false }) : "—"
          }</td>
        </tr>`;
      })
  ...
  async function bootDaily() {
    ...
    __DAILY_INDEX__ = index;
    if (index.generated_at) {
      $all("[data-asof]").forEach((el) => {
        el.textContent = formatAsOf(index.generated_at);
      });
    }
    populateDailySelect(index);
    const sel = $("#daily-day");
    const day = (sel && sel.value) || defaultDailyDay(index);
    if (day) {
      try {
        const url = new URL(location.href);
      ...
      await loadAndRenderDailyDay(day);
    }
    await bootNavSeries();
  }
