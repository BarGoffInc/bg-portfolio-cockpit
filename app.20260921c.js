...
(function () {
  ...
  function renderNavSeriesChart(series) {
    ...
    if (sub) {
      ...
      sub.textContent = n
        ? n + " NAV mark" + (n === 1 ? "" : "s") + (last ? " · latest " + last.day + " " + fmtCompactUSD(last.nav_usd) : "")
        : "No trusted full-book marks yet";
    }
    ...
    // X axis: use calendar span of NAV points (and flows if shown)
    ...
    // If few NAV points, still space them across the plot by day index
    const dayIndex = {};
    ...
    if (__NAV_SHOW_FLOW__) {
      maxFlowAbs = Math.max(
        1,
        ...flowDays.map((d) => Math.abs(Number(d.net_flow_usd) || 0))
      );
    }
    const yFlow = (v) => {
    ...
    // Day P/L bars (only where day_pnl known)
    ...
    const barW = Math.max(6, Math.min(28, plotW / Math.max(points.length * 2, 4)));
    points.forEach((p) => {
      ...
      pnlBars += `<rect class="nav-pnl-bar" data-day="${p.day}" x="${(x - barW / 2).toFixed(1)}" y="${top.toFixed(1)}" width="${barW.toFixed(1)}" height="${h.toFixed(1)}" rx="2" fill="${fill}" opacity="0.85"/>`;
    });
    ...
    if (__NAV_SHOW_FLOW__) {
      ...
      const fBarW = Math.max(3, Math.min(10, plotW / Math.max(flowDays.length, 8)));
      flowDays.forEach((d) => {
    ...
    points.forEach((p) => {
      ...
      const dash = p.provisional ? ` stroke-dasharray="3 2"` : "";
      hits += `<circle class="nav-dot" data-day="${p.day}" cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="4.5" fill="#147b83" stroke="#fff" stroke-width="1.5"${dash}/>`;
      hits += `<rect class="nav-hit" data-day="${p.day}" x="${(x - 14).toFixed(1)}" y="${navTop}" width="28" height="${navH + pnlH + 20}" fill="transparent"/>`;
    });

    // X labels for NAV days
    let xLabels = "";
    points.forEach((p) => {
      const x = xAt(p.day);
    ...
    const labels = `
      ...
      <text x="${pad.l}" y="${(pnlTop - 2).toFixed(1)}" fill="#5b6b73" font-size="10" font-weight="600" font-family="ui-sans-serif,system-ui">Day P/L (vs prior mark)</text>
    ...
    svg.innerHTML =
      labels +
      grid +
      `<path d="${poly}" fill="none" stroke="#147b83" stroke-width="2.25" stroke-linejoin="round" stroke-linecap="round"/>` +
      pnlBars +
      flowBars +
      hits +
      xLabels;
    ...
    function showTip(day, clientX, clientY) {
      ...
      tip.innerHTML =
        ...
        (p.provisional ? `<span class="text-muted">provisional</span>` : "") +
        (f && __NAV_SHOW_FLOW__
