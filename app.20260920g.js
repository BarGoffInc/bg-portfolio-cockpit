...
(function () {
  ...
  async function hardRefresh(ev) {
    ...
    toast("Reloading saved book…", 8000);
    try {
      // IMPORTANT: do not client-side reprice. Live marks via Coinbase were
      // inflating Hyperliquid perps (notional ≠ equity) and confusing weekend stocks.
      // Full live sync = 3-hour cockpit job (and on-demand from Conviction).
      ...
      const total = book.totals && book.totals.book_usd != null ? fmtUSD(book.totals.book_usd, { cents: false }) : "";
      toast("Reloaded " + total + " · " + when + " · full live sync every 3 hours");
    } catch (err) {
      console.error(err);
